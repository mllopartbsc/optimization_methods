...
def generate_forked(sentence, model, new_speaker_ids, use_aliases, speaker_id):
    ...
    if pid == 0:
        ...
        model.save_wav(wavs, f"./test/{datetime.utcnow()}.wav")

        os._exit(0)  # Exit the child process after using the model
...
def generate_normal(sentence, model, new_speaker_ids, use_aliases, speaker_id):
    print(f"Processing sentence: {sentence}")
    print(" > Model input: {}".format(sentence))
    print(" > Speaker Idx: {}".format(speaker_id))
    if use_aliases:
...
print("model loaded, sleeping 7 seconds")
...
print("Start of first inference forked")

start_time = time.time()

this = generate_forked(sentence=text1, model=syn, new_speaker_ids=new_speaker_ids, use_aliases=None, speaker_id=input_speaker_id)

end_time = time.time()

elapsed_time = end_time - start_time

print("first inference, forked, took", elapsed_time)

print("first inference sleeping 7 seconds")

print("Start of first inference non-forked")

start_time = time.time()

this = generate_normal(sentence=text1, model=syn, new_speaker_ids=new_speaker_ids, use_aliases=None, speaker_id=input_speaker_id)

end_time = time.time()

elapsed_time = end_time - start_time

print("first inference, normal, took", elapsed_time)
...
# print("Start of second inference")
#
...
#
# this = generate_normal(sentence=text2, model=syn, new_speaker_ids=new_speaker_ids, use_aliases=None, speaker_id=input_speaker_id)
#
# print("first inference took" + elapsed_time)
#
# print("Second inference, sleeping 7 seconds")
#
# time.sleep(7)
#
# print("Start of third inference")
#
#
...
#
# this = generate_forked(sentence=text3, model=syn, new_speaker_ids=new_speaker_ids, use_aliases=None, speaker_id=input_speaker_id)
#
# print("third inference, sleeping 7 seconds")
#
# time.sleep(7)
#
# print("Start of fourth inference")
#
# text4 = "El procés de renaixença o ressorgiment cultural és un fenomen que es pot observar en moltes cultures al llarg de la història, on una societat reviu, redescobreix o revaloritza la seva identitat, llengua i tradicions. A Catalunya, aquest procés és conegut com la Renaixença, i es va produir durant el segle XIX, sent una resposta directa a la repressió i la centralització que havia sofert el país durant segles sota diferents dominacions. Des dels Decrets de Nova Planta, promulgats per Felip V després de la Guerra de Successió, Catalunya va veure com les seves lleis i institucions particulars eren abolides. Això va provocar una gradual castellanització de la societat catalana, reprimint la llengua i la cultura pròpies. Amb el temps, però, aquesta situació va generar una resposta cultural i nacionalista que buscava reivindicar l'identitat catalana. Durant la Renaixença, es va produir un auge de la llengua catalana com a eina de creació literària. Poetes i escriptors com Jacint Verdaguer, Àngel Guimerà i Joan Maragall van jugar un paper clau en la revitalització del català com a llengua literària. La publicació de l'obra L'Atlàntida de Verdaguer va ser un punt d'inflexió, demostrant la riquesa i potencial del català com a llengua de gran literatura. Paral·lelament a la literatura, hi va haver un esforç per recuperar i promoure les festes, la música i les tradicions populars catalanes."
# this = generate_normal(sentence=text4, model=syn, new_speaker_ids=new_speaker_ids, use_aliases=None, speaker_id=input_speaker_id)
#
# print("finl output")
#
# time.sleep(10)